...
export class RecipeToolsComponent implements OnInit {
  ...
  orders : order[] = this.RecipesService.getOrders()
  ...
  {
    if(this.orders.find( el => el.name == value)){
  ...
  removeOrder(value : String){
    this.orders = this.orders.filter(o => o.name != value)
    this.totalOrders = this.calcTotal();
  }
